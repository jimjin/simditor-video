(function (root, factory) {
  if ((typeof define === 'function') && define.amd) {
    ...
    root['VideoButton'] = factory(jQuery,Simditor);
  }
}(this, function($, Simditor) {
  Simditor.i18n['zh-CN'].video = '插入视频';
  Simditor.i18n['zh-CN'].videoSize = '视频尺寸(可以百分比)';
  Simditor.i18n['zh-CN'].videoPlaceholder = '粘贴视频地址到这里';
  Simditor.i18n['zh-CN'].uploadVideoBtn = '确认插入视频';
  ...
  VideoButton = (function(superClass) {
    ...
    VideoButton.prototype._init = function() {
      VideoButton.__super__._init.call(this);
      ...
      $(document).off('click', '.J_UploadVideoBtn');
      $(document).on('click', '.J_UploadVideoBtn', (function(_this) {
        return function(e) {
          var videoData;
          videoData = {
            link: $('.video-link').val(),
            width: $('#video-width').val() || 400,
            height: $('#video-height').val() || 300
          };

          $('.video-link').val('');
          $('#video-width').val('');
          $('#video-height').val('');
          _this.loadVideo($('.J_UploadVideoBtn').data('videowrap'), videoData, function() {
    ...
    VideoButton.prototype.loadVideo = function($video, videoData, callback) {
      ...
      if (!videoData.link && !$video.attr('data-link')) {
        ...
        $video.attr({
          'data-link': videoLink,
          'data-tag': videoTag,
          'width': videoData.width || 400,
          'height': videoData.height || 300
        });
